...
export function TemplateMailTablet({ stopConfetti }: propFather) {
    ...
    const updateButtonActivation = () => {
        ...
        console.log(isEmailValid, isChecked, isCheckedFirst)
        ...
        try {
            ...
            switch (data.status_code) {
                ...
                case 403:
                    ...
                    setDataModal({ title: 'Debes jugar para poder recibir el premio', subTitle: 'Se refrescará la pagina y volverá al inicio para poder jugar' })
                    break;
                case 405:
                    setModal({ boolean: true, number: 405 })
                    setDataModal({ title: "Ocurrió un error en el campo requerido(mail)", subTitle: 'Intentelo nuevamente' })
                    break;
                case 404:
                ...
                case 401:
                    ...
                    setDataModal({ title: 'Por alguna razón el formulario ya ha sido enviado', subTitle: 'Se lo redirigirá a otra section' })
                    break;
                default:
                    break;
            }
    ...
    return (
        <div style={{ zIndex: 10 }} className=" tracking-wide z-50 mx-[2rem]  py-[6rem] textGothamMedium">
            ...
            <h1 style={{ textShadow: '2px 4px 4px rgba(0, 0, 0, 0.5)' }} className="text-2xl font-bold  text-white text-center sm:text-4xl ">INGRESÁ TU CORREO Y TE ENVIAREMOS UN MAIL CON TU PREMIO</h1>
            <h2 className="text-yellowMain mb-8 mt-4 sm:my-12  text-sm sm:text-xl">
            ...
            <button
                ...
                className={`${buttonActivated ? 'bg-gradient-to-r from-green-700 to-greenMain' : 'bg-green-700 bg-opacity-70 text-opacity-70 '} uppercase rounded-3xl p-2 px-[3rem] cursor-pointer  text-white sm:text-xl`}
            >
                Enviar
            </button>
            <div className=" flex justify-center my-[2rem] ">
                <img src={logo} className=" w-[12rem] sm:w-[16rem]" alt="logo empresarial" />
            </div>
